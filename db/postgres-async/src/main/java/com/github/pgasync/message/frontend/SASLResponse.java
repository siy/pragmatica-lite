...
public class SASLResponse implements Message {
    ...
    private SASLResponse(String password, String serverSalt, int i, String serverNonce, String gs2Header, String clientFirstMessageBare, String serverFirstMessage) {
    ...
    public String clientFinalMessage(String hMacName, String digestName) {
        String clientFinalMessageWithoutProof = "c=" + new String(Base64.getEncoder().withoutPadding().encode(gs2Header.getBytes(StandardCharsets.US_ASCII)), StandardCharsets.UTF_8)
                + ",r=" + serverNonce;
        return clientFinalMessageWithoutProof
                + ",p=" + saslClientProof(password, serverSalt, i, clientFirstMessageBare, serverFirstMessage, clientFinalMessageWithoutProof, hMacName, digestName);
    }
    ...
    private static String saslClientProof(String password, String salt, int i, String clientFirstMessageBare, String serverFirstMessage, String clientFinalMessageWithoutProof, String hMacName, String digestName) {
        try {
    ...
    public static SASLResponse of(String password, String serverFirstMessage, String clientNonce, String gs2Header, String clientFirstMessageBare) {
        Authentication.SaslContinueServerFirstMessage continueData = Authentication.SaslContinueServerFirstMessage.parse(serverFirstMessage);
        String serverNonce = continueData.getAugmentedNonce();
        if (serverNonce != null &&
                serverNonce.length() > clientNonce.length() &&
                serverNonce.startsWith(clientNonce)) {
            return new SASLResponse(password, continueData.getSalt(), continueData.getIterations(), serverNonce, gs2Header, clientFirstMessageBare, serverFirstMessage);
        } else {
