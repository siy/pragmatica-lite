...
public class ArrayConversionsTest {
    ...
    private Row getRow() {
        return dbr.query("SELECT * FROM CA_TEST").unwrap().index(0);
    }
    ...
    public void selectNestedInt() {
        ...
        assertArrayEquals(
                ...
                dbr.query(
                        ...
                        List.of(new Object[]{a})).unwrap().index(0).getArray("inta", Integer[].class));
    }

    @SuppressWarnings("deprecation")
    @Test
    public void selectUTF8() {
        var a = new String[]{"U&\"d\\0061t\\+000061\"", "d\u0061t\u0061"};
        ...
        assertArrayEquals(
                ...
                dbr.query(
                        ...
                        List.of(new Object[]{a})).unwrap().index(0).getArray("texta", String[].class));
    }

    @SuppressWarnings("deprecation")
    @Test
    public void selectUnboxed() {
        var a = new short[][]{new short[]{0, 1}, new short[]{1, 0}};
        ...
        assertEquals(
                ...
                dbr.query(
                        "SELECT INTA FROM CA_TEST WHERE INTA = $1",
                        List.of(new Object[]{a})).unwrap().size());
    }
    ...
    public void implicitGet() {
        ...
        var row = (PgRow) dbr.query("SELECT * FROM CA_TEST").unwrap().index(0);
    ...
    public void shouldRoundTripTimestamp() {
        var params = List.<Timestamp[]>of(new Timestamp[] { new Timestamp(12345679), new Timestamp(12345678) } );

        dbr.query("INSERT INTO CA_TEST (TIMESTAMPA) VALUES ($1)", params);

        var row = dbr.query("SELECT TIMESTAMPA FROM CA_TEST WHERE TIMESTAMPA = $1", params).unwrap().index(0);

        assertArrayEquals(params.getFirst(), row.getArray(0, Timestamp[].class));
    }
    ...
    public void shouldParseUnquotedStringsCorrectly() {
        ...
        var row = dbr.query("SELECT * FROM CA_TEST").unwrap().index(0);
    ...
    public void shouldBindArrayOfByteA() {
        ...
        var readbb = dbr.query("SELECT BYTEAA FROM CA_TEST WHERE BYTEAA = $1", Collections.singletonList(bb))
                        .unwrap().index(0).getArray(0, byte[][].class);
