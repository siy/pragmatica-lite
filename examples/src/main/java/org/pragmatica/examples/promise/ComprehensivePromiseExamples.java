...
public class ComprehensivePromiseExamples {
    ...
    public void errorHandlingPatterns() {
        ...
        Promise<String> withRecovery = riskyOperation.recover(error -> "Recovered from: " + error.message());
        // Transform errors
        Promise<String> withErrorTransform = riskyOperation.mapError(error -> CoreError.exception(new IllegalStateException("Wrapped: " + error.message())));
        // Provide fallback
        Promise<String> withFallback = riskyOperation.orElse(() -> Promise.resolved(Result.success("Default value")));
    }
    ...
    public void compositionPatterns() {
        ...
        Promise<String> sequential = Promise.resolved(Result.success("Start"))
                                            ...
                                            .flatMap(step2 -> Promise.promise(() -> Result.success(step2
                                                                                                   + " -> Complete")));
    }
    ...
    public void parallelProcessing() {
        ...
        // Process results
        Promise<Integer> sum = allResults.map(results -> results.stream()
