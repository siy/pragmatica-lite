...
public interface UserRegistration1 {
    ...
    static UserRegistration1 userRegistration(MockUserDataStore userDataStore,
                                              PasswordHasher passwordHasher,
                                              JwtTokenGenerator jwtTokenGenerator) {
        record UserRegistrationImpl(MockUserDataStore userDataStore, PasswordHasher passwordHasher,
                                    JwtTokenGenerator jwtTokenGenerator) implements UserRegistration1 {
            @Override
            public Promise<Response> perform(Request request) {
                return ParsedRequest.parse(request).async()
                                    .flatMap(this::checkEmailIsUnique)
                                    .flatMap(this::checkUsernameIsUnique)
                                    .flatMap(this::hashPasswordAndCreateUser)
            ...
            private Promise<RegisteredUser> hashPasswordAndCreateUser(ParsedRequest request) {
