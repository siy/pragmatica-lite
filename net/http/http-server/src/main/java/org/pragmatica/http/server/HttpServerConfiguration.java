...
public record HttpServerConfiguration(int port, Option<InetAddress> bindAddress, int sendBufferSize,
                                      int receiveBufferSize, int maxContentLen, boolean nativeTransport,
                                      Option<CustomCodec> customCodec,
                                      Option<JsonCodec> jsonCodec,
                                      Option<SslContext> sslContext, Option<CorsConfig> corsConfig) implements HttpServerConfigurationTemplate.With {
    ...
    public static HttpServerConfiguration defaultConfiguration() {
        return new HttpServerConfiguration(DEFAULT_PORT, none(), DEFAULT_SEND_BUFFER_SIZE, DEFAULT_RECEIVE_BUFFER_SIZE,
