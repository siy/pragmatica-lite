...
```
(TODO independent actions examples)

## Predicates

For each monad there are implementations of `all()` and `any()` predicates, which provide a convenient tool for handling several monads at once.
The API is carefully designed to be as convenient as possible, while still being type-safe and consistent.
...
```
Again, it is recommended to use method references and avoid creating intermediate variables unless strictly necessary:
```java
Option<SomeType> method(SomeType1 parameter1, SomeType2 parameter2, SomeType3 parameter3) {
...
The `map()` method behaves similar to `map()` method of corresponding monad, but it receives all values at once. The result of the `map()` method is a new `Option<T>` instance with the result of transformation of all three values. The `flatMap()` method behaves identically except transformation function returns `Option<T>`.

The `all()` predicate enables expression in code that computation of intermediate values are independent from each other. Traditional approach with sequential computation of each value does not provide such capability. 

Both (`all()` and `any()`) predicates in when applied to `Promise<T>` enable effortless parallelization of computations using `fan out -> fan in` pattern:
...
```
Note that invoked methods return `Promise<T>` which can still be awaiting completion of the underlying action. The `map()` method also returns not yet resolved promise. In other words, all processing is performed in non-blocking manner. When all values will be available and all involved promises will be resolved to successful result, the computation passed to `map()` will be executed.
